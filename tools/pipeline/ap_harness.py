...
Reads the shards from `data/wildfire-spread/` and, when present, `static_coarse.npz`
...
trust it. The shard list is checked against report.json, so a stale or mixed
directory is refused rather than scored.
...
def cell_offsets(cur, cell_deg, lat_top):
    """Each cell's offset from the fire's centroid, its distance in km, and a flag.

    `dist` uses the true per-axis spacing: a cell is `cell_deg * 110.54` km tall and
    `cell_deg * 111.32 * cos(lat)` km wide, so it shrinks towards the north.

    When the current window holds no fire the centroid is undefined. Returning the
    offsets anyway would read as the top-left grid corner (dr=dc=64 at the centre),
    a value half the samples a corner one in seven. Those offsets are zeroed and the
    `has_fire` flag carries the distinction instead.
    """
    n, h, w = cur.shape
    rr, cc = np.mgrid[0:h, 0:w].astype(np.float32)
    tot = cur.sum(axis=(1, 2))
    has_fire = tot > 0
    safe = np.where(has_fire, tot, 1.0)
    cr = (cur * rr).sum(axis=(1, 2)) / safe
    cc_ = (cur * cc).sum(axis=(1, 2)) / safe
    dr = rr[None] - cr[:, None, None]
    dc = cc[None] - cc_[:, None, None]
    km_row = cell_deg * 110.54                                             # (n,)
    lats = lat_top[:, None] - (np.arange(h)[None, :] + 0.5) * cell_deg[:, None]
    km_col = cell_deg[:, None] * 111.32 * np.cos(np.radians(lats))         # (n, h)
    dist = np.hypot(dr * km_row[:, None, None], dc * km_col[:, :, None])
    keep = has_fire[:, None, None]
    return (np.where(keep, dr, 0.0), np.where(keep, dc, 0.0),
            np.where(keep, dist, 0.0), has_fire.astype(np.float32))
...
def verify_manifest(shards, report_path: Path) -> None:
    """Refuse a shard set that does not match report.json's list.

    build.py names shards by index, so a re-run into the same directory can leave a
    stale tail behind. Scoring the mixture would print a confident table for a
    dataset the report does not describe.
    """
    if not report_path.exists():
        print(f"note: {report_path} absent — cannot check the shard manifest\n")
        return
    listed = [Path(s["file"]).name for s in json.loads(report_path.read_text())["shards"]]
    found = [Path(s).name for s in shards]
    if found != listed:
        raise SystemExit(
            "shards on disk do not match report.json:\n"
            f"  disk:   {found}\n  report: {listed}\n"
            "Rebuild with build.py, or point --shards and --report at one run."
        )
...
def load(shards, static):
    ...
    for f in shards:
        ...
        lab = d["label_frp"]
        n, h, w = cur.shape
        dr, dc, dist, has_fire = cell_offsets(cur, d["cell_deg"].astype(np.float32),
                                              (d["lat0"] + h * d["cell_deg"]).astype(np.float32))
        has_fire = np.broadcast_to(has_fire[:, None, None], (n, h, w))
        blocks["fire"].append(np.stack([cur, curd, hist, histd], -1).astype(np.float32).reshape(n * h * w, 4))
        blocks["offset"].append(np.stack([dr, dc, dist, has_fire], -1).astype(np.float32).reshape(n * h * w, 4))
...
def score_variants(X, y, g, pers, drift, rng):
    ...
    for tr, te in GroupKFold(n_splits=N_SPLITS).split(X["fire"], y, g):
...
def main() -> int:
    ...
    print(f"shards: {len(shards)} from {Path(shards[0]).resolve().parent}")
    verify_manifest(shards, args.report)
    ...
    print(f"cells={len(y):,}  positives={int(y.sum()):,}  "
          f"prevalence={100 * y.mean():.4f}%  events={len(groups)}\n")
    if len(groups) < N_SPLITS:
        raise SystemExit(f"only {len(groups)} event group(s); GroupKFold({N_SPLITS}) needs "
                         f"{N_SPLITS}. The dataset is too small to score.")
    ...
    def line(label, value):
        ratio = "degenerate" if persist == 0 else f"{value / persist:.2f}x"
        print(f"  {label:<54} AP {value:.4f}   {ratio}")

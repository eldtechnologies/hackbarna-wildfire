...
def main():
    ...
    spec = importlib.util.spec_from_file_location(
        "tools.next_run.paired_inputs", REPO / "tools/next_run/inputs.py"
    )
    ...
    rows = json.loads((OUT / "eligible.json").read_text())
    for path, key in [
    ...
    saved = torch.load(
        BASE / "next-run-data/run-001/frozen.pt", map_location="cpu", weights_only=False
    )
    ...
    for row in prepared:
        ...
        for hi, h in [(1, 3), (2, 6)]:
            ...
            dlow = raster(sim, lo, r0, c0)
            dhigh = raster(sim, high, r0, c0)
            predictions = {

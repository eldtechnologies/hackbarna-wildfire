...
const frame:FiresResponse={provenance:'replay',scenario:'capture',fetchedAt:'2026-09-19T01:00:00Z',asOf:'2026-09-19T01:00:00Z',
  source:'configured',requestedSource:'configured',hotspots:[],clusters:[],perimeters:[],spread:[],
  timeline:{scenario:'capture',start:'2026-09-19T00:00:00Z',end:'2026-09-19T01:00:00Z',durationSeconds:3600,frames:[]}};
...
test('source selector reports actual provenance and keeps failure visible without relabelling the old scene', async t=>{
  ...
  assert.deepEqual([...root.querySelectorAll('option')].map(option=>option.value),['live','replay','configured']);
  assert.equal(root.querySelector('select')!.value,'configured');
  assert.match(root.textContent!,/REPLAY · recorded satellite observations/);
  fail=true;
  ...
  assert.equal(playback.getState().data?.source,'configured');
  assert.match(root.querySelector('.source-status')!.textContent!,/Source unavailable.*REPLAY · recorded satellite observations/);
});
...
test('committed source is forwarded to both reports, including same fire/time after source changes', async t=>{
  ...
  const calls:unknown[][]=[];
  const playback=new FirePlayback(async(_at,_signal,source)=>({...frame,source:source??'configured'}));
  ...
  assert.deepEqual(calls.map(args=>args[3]),['configured','configured','replay','replay']);
...
test('fire, threat and report HTTP clients propagate source alongside the zero cursor', async t=>{
  ...
  await fetchFires(0,undefined,'configured');await fetchThreats('fire/a',0,undefined,'configured');await fetchSituation('fire/a',0,undefined,'configured');
  for(const url of urls){assert.match(url,/at=0/);assert.match(url,/source=configured/);}
});
